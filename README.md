...
Проект размещен по ссылке: https://yl-flask-project.herokuapp.com (некоторые функции могут не работать из-за ограничений хостинга)\
...
При patch запросах будут изменены переданные поля у переданного id, поля, которые не были переданы, изменены не будут
...
Для полей по ключам в post/patch api запросах есть свои ограничения:
...
—country_from: id страны должен существовать в БД \
...
—creator_id: id пользователя должен существовать в БД \
—participating: id пользователей должны существовать в БД ; creator_id не может находиться в этом списке \
—country: id страны должен существовать в БД \
—type: id типа должен существовать в БД
...
—При запуске сервера в файл current_time_zone.txt записывается текущий часовой пояс машины, с которой был произведен запуск. Данный часовой пояс будет отображен на странице редактирования и создания мероприятия, а также на странице просмотра подробной информации о мероприятии \
